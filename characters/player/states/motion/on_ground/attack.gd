...
func _on_Weapon_attack_started(host):
	...
	if sign(vector.x) == -1 or (host.look_direction.x == -1 and vector.x == 0 and vector.y != 0): # host is facing left, requires input_direction so default doesn't face right
		angle -= 180
