...
func follow(host, velocity, target_position, max_speed):
	var desired_velocity = (target_position - host.global_position).normalized() * max_speed

	var push = calculate_avoid_force(host, desired_velocity)
	var steering = (desired_velocity - velocity + push) / MASS

	return velocity + steering


func arrive_to(host, velocity, target_position, slow_radius=DEFAULT_SLOW_RADIUS, max_speed=DEFAULT_MAX_SPEED, avoid=false):
