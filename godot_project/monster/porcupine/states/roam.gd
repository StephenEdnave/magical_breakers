...
func update(host, delta):
	host.velocity = host.arrive_to(host.velocity, roam_target_position, roam_slow_radius, max_roam_speed)
	host.move_and_slide(host.velocity)
	if host.position.distance_to(roam_target_position) < host.ARRIVE_DISTANCE:
		return IDLE
	elif host.position.distance_to(host.target_position) < host.SPOT_RANGE:
